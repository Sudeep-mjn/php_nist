...
try {
    ...
    $notices_count = $stmt->fetch()['count'] ?? 0;
    
    // Count activities
    $stmt = $db->query("SELECT COUNT(*) as count FROM activities");
    $activities_count = $stmt->fetch()['count'] ?? 0;
                        ...
                        <div class="space-y-4">
                            <?php foreach ($recent_notices as $notice): ?>
                                <div class="border-l-4 border-blue-500 pl-4">
                                    ...
                                    <p class="text-sm text-gray-600 mt-1"><?php echo htmlspecialchars(substr($notice['description'], 0, 100)) . '...'; ?></p>
                                    <p class="text-xs text-gray-400 mt-1"><?php echo date('M j, Y', strtotime($notice['date'])); ?></p>
                                </div>
                            <?php endforeach; ?>
                        </div>
                    <?php endif; ?>
